...
export async function createProject(formData: FormData) {
  ...
  const schema = z.object({ title: z.string().trim().min(1).max(80), aspect: z.enum(["reel", "portrait", "square"]), template: z.string().max(40) })
  const data = schema.parse(Object.fromEntries(formData))
  const dimensions = { reel: [1080, 1920], portrait: [1080, 1350], square: [1080, 1080] }[data.aspect]
  const [project] = await db.insert(projects).values({ userId: user.id, title: data.title, status: "draft", width: dimensions[0], height: dimensions[1], frameRate: 30, duration: 15, editorState: { ...baseState, template: data.template }, revision: 1 }).returning()
